...
| ⟨sfc, prf, _⟩ := 
  ...
  | [(rhs, cr), (lhs, cl)] := 
    if lhs.lt rhs then mk_ineq_data_of_lhs_rhs lhs rhs cl cr (sfc.c.to_comp) prf
...
do il ← get_ineq_list,
   return $ il.map (λ ⟨lhs, rhs, id⟩, sum_form_comp_data.of_ineq_data id) 
   
private meta def reduce_option_list {α} : list (option α) → list α
| [] := []
